    ...
    func segmentContentSize(_ segment: WBInnerSegment) -> CGSize {
        ...
        case let .text(text):
            size = (text as NSString).size(withAttributes: [
                kCTFontAttributeName as NSAttributedStringKey: segmentTextBold ? UIFont.boldSystemFont(ofSize: self.segmentTextFontSize) : UIFont.systemFont(ofSize: self.segmentTextFontSize)
                ])
        case let .icon(icon):
            size = icon.size
        }
